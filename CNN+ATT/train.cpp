...
void
train_gradient(int *sentence, int *trainPositionE1, int *trainPositionE2, int len, int e1, int e2, int r1, float alpha,
               vector<float> &r, vector<int> &tip, vector<float> &grad) {
  for (int i = 0; i < dimensionC; i++) {
    if (fabs(grad[i]) < 1e-8)
      continue;
    int last = i * dimension * window;
...
float train_bags(string bags_name) {
  ...
  for (int k = 0; k < bags_size; k++) {
    int i = bags_train[bags_name][k];
    train_gradient(trainLists[i], trainPositionE1[i], trainPositionE2[i], trainLength[i], headList[i], tailList[i],
                   relationList[i], alpha1, rList[k], tipList[k], grad[k]);
...
int main(int argc, char **argv) {
  ...
  init();
  //for (map<string,vector<int> >:: iterator it = bags_train.begin(); it!=bags_train._end(); it++)
  //	cout<<it->first<<endl;
